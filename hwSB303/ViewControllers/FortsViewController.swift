...
final class FortsViewController: UITableViewController {
    private var data : AllFortsSecResponse?
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let securities = self.data?.securities else { return 0 }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let securities = data?.securities, let marketdata = data?.marketdata else { return UITableViewCell() }
